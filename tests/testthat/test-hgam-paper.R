...
test_that("draw() can plot CO2 model 1", {
    skip_on_cran()
    skip_on_ci()
    CO2_mod1 <- bam(log(uptake) ~ s(log(conc), k = 5, bs = "tp") +
...
test_that("draw() can plot CO2 model 2", {
    ...
    skip_on_ci()
    expect_warning(
        CO2_mod2 <- bam(log(uptake) ~ s(log(conc), k = 5, m = 2) +
...
test_that("draw() can plot CO2 model 5", {
    skip_on_cran()
    skip_on_ci()
    CO2_mod5 <- bam(log(uptake) ~ s(log(conc), by = Plant_uo, k = 5, bs = "tp",
                                    m = 2) +
                        s(Plant_uo, bs = "re", k = 12),
                    data = CO2, method = "fREML",
                    control = ctrl)
...
test_that("draw() can plot bird_move model 1", {
    skip_on_cran()
    skip_on_ci()
    bird_mod1 <- bam(count ~ te(week, latitude, bs = c("cc", "tp"),
                                k = c(10, 10)),
...
test_that("draw() can plot bird_move model 2", {
    ...
    skip_on_ci()
    expect_warning(
    bird_mod2 <- bam(count ~ te(week, latitude, bs = c("cc", "tp"),
                                k = c(10, 10), m = 2) +
...
test_that("draw() throws message with bird_move model 4", {
    ...
    skip_on_ci()
    expect_warning(
        bird_mod4 <- bam(count ~ t2(week, latitude, species,
            bs = c("cc", "tp", "re"),
...
test_that("draw() can plot zoo_comm_mod model 4", {
    skip_on_cran()
    skip_on_ci()
    zoo_comm_mod4 <- bam(density_adj ~ s(day, taxon,
                                         bs = "fs",
                                         k = 10,
                                         xt = list(bs = "cc")) +
...
test_that("draw() can plot zoo_comm_mod model 5", {
    skip_on_cran()
    skip_on_ci()
    zoo_comm_mod5 <- bam(density_adj ~ s(day, by = taxon,
                                         k = 10, bs = "cc") +
                                   s(taxon, bs = "re") +
                                   s(taxon, year_f, bs = "re"),
                     data = zoo_train,
