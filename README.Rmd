...
```{r draw-gam-figure, dev = "png", fig.height = 6, fig.width = 7}
library('mgcv')
library('gratia')
...
```{r apprise-figure, dev = "png", fig.height = 6, fig.width = 7}
appraise(m)
```
...
* A *ggplot2*-based replacement for `mgcv:::plot.gam()`: `draw(gam_model)`.
    ...
    ![Estimated smooths from a GAM](man/figures/README-draw-gam-figure.png)
...
* Model diagnostics via `appraise()`
    ...
    ![Model diagnostics figure](man/figures/README-appraise-figure.png)
